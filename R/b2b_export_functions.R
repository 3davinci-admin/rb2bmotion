...
b2b_export <- function(b2b_site, method, filters = NULL, limit = 100) {
    ...
    totalCount <- b2b_site %>% api_new_url(method = method, limit = 1, filters = filters) %>% 
        api_get_response() %>% content("text") %>% fromJSON() %>% .[["totalCount"]]
    ...
    for (page in 1:page_num) {
        ...
        tbl_list[[page]] <- b2b_site %>% api_new_url(method = method, limit = limit, 
            offset = offset, filters = filters) %>% api_get_response() %>% api_tidy_response()
